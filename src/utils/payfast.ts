export function md5(string: string): string {
  ...
  function ConvertToWordArray(str: string) {
    ...
    const lBytePositionFinal = (lByteCount % 4) * 8;
    lWordArray[lWordCountFinal] = lWordArray[lWordCountFinal] | (0x80 << lBytePositionFinal);
...
export const payFastGlobalConfig: PayFastConfig = {
  merchant_id: "34949677",
  merchant_key: "nuuiq9dqs7jut",
  passphrase: "Sithembile.1234",
  is_live: true
};

export interface PayFastPaymentFields {
  merchant_id: string;
  merchant_key: string;
  return_url: string;
  cancel_url: string;
...
export function generatePayFastSignature(
  data: PayFastPaymentFields,
  passphrase?: string
): string {
  // Ordered fields exactly as per PayFast rules
  const keys = [
    'merchant_id',
    'merchant_key',
    'return_url',
  ...
  if (passphrase && passphrase.trim() !== '') {
    ...
    str += `&passphrase=${encodedPassphrase}`;
  }

  return md5(str);
}

export function getPayFastUrl(isLive = true): string {
  return isLive 
